...
function parseInput(value: unknown): MatchInput {
  ...
  return { matchDate, competition, venue, state, payloadJson: boundedJson(source.payload ?? {}, 512_000), events };
}
...
export async function listMatches(request: Request, env: Env, auth: AuthContext, clubId: string, requestId: string): Promise<Response> {
  ...
  const query = cursor
    ? env.DB.prepare(`SELECT id,match_date AS matchDate,competition,venue,state,payload_json AS payloadJson,version,created_at AS createdAt,updated_at AS updatedAt
...
export async function createMatch(request: Request, env: Env, auth: AuthContext, clubId: string, requestId: string): Promise<Response> {
  ...
  const statements: D1PreparedStatement[] = [
    env.DB.prepare(`INSERT INTO matches (club_id,id,match_date,competition,venue,state,payload_json,version,created_at,updated_at)
      VALUES (?,?,?,?,?,?,?,1,?,?)`).bind(context.clubId, id, input.matchDate, input.competition, input.venue, input.state, input.payloadJson, now, now),
    ...input.events.map((event) => env.DB.prepare(`INSERT INTO match_events (club_id,id,match_id,event_type,match_ms,payload_json,created_at,updated_at)
      VALUES (?,?,?,?,?,?,?,?)`).bind(context.clubId, event.id, id, event.eventType, event.matchMs, event.payloadJson, now, now)),
  ];
