...
export async function exportClub(request: Request, env: Env, auth: AuthContext, clubId: string, requestId: string): Promise<Response> {
  ...
  const [clubRow, teams, players, matches, events, tournaments, members, imports, audit] = await Promise.all([
    ...
    env.DB.prepare("SELECT id,team_id AS teamId,external_id AS externalId,first_name AS firstName,last_name AS lastName,name,shirt_number AS shirtNumber,pass_number AS passNumber,birthdate,created_at AS createdAt,updated_at AS updatedAt FROM players WHERE club_id=? ORDER BY team_id,name").bind(club).all(),
